...
export function gymValidate(newGym) {
  ...
  } else if (!Number(newGym.price)) {
    error.price = "Solo se puede ingresar numeros enteros";
  } else if (!Number(newGym.phone)) {
    error.phone =
...
export function gymValidateEdit(editGym) {
  let error = {};
  let pric = editGym.price === "" ? 999 : editGym.price;
  let phon = editGym.phone === "" ? 123456789 : editGym.phone;
  let emai = editGym.email === "" ? "correo@ejemplo.com" : editGym.email;
  console.log(pric)
  console.log(phon)
  if (!regexName.test(editGym.name)) {
    error.name = "El nombre es requerido y debe tener por lo menos dos letras";
